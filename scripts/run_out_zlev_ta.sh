#!/bin/sh
#SBATCH --job-name=wrf-zta
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=6:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-zta.%j.out
#SBATCH --error=wrf-zta.%j.out
...
cd ${RUN_DIR}
#
#  Time Loop (yeari <= yearf; it is ustally =)                            |
#
...
# 2. Compile program
#
   $FC $FFLAGS ${PROG_DIR}/RCM_zlev_ta.f90 ${MOD_NAME}.o ${SUB_NAME}.o -o RCM_zlev_ta.exe $ALL_LIBS
...
# sbatch --file=slurm_common.opts run_loop_zlev_ta.sh ${datebeg} ${dateend} ${year_lim}
 sbatch --file=slurm_common.opts run_out_plev_ta.sh ${datebeg} ${dateend} ${year_lim}
fi
