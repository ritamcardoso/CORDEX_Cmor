...
#SBATCH --job-name=loop-wpth
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=50:00
#SBATCH --hint=nomultithread
#SBATCH --output=loop-wpth.%j.out
#SBATCH --error=loop-wpth.%j.out
...
#declare -a var=("prw" "clwvi" "clivi")
declare -a var=("clwvi")

RUN_DIR=$SCRATCH/WRF/clwvi
...
if [ $yeari -le 1993 ]; then

  sbatch run_out_clwvi.sh ${datebeg} ${dateend}
...
for (( v=0; v<${#var[@]}; v++)); do
  ...
  scp ${var[$v]}_*.nc wrf@vortex.degge.fc.ul.pt:/media/tor_disk2/data/wrf/CORDEX_CMIP6/output/${var[$v]}
