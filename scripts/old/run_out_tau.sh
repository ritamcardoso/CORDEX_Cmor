#!/bin/sh
#SBATCH --job-name=wrf-tau
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=12:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-tau.%j.out
#SBATCH --error=wrf-tau.%j.out
...
#                           TO CHANGE                           |
#----------------------------------------------------------------
RUN_DIR=$SCRATCH/ssp370/tau
mkdir -p ssp370/tau

declare -a run=("d01")
declare -a var=("tauu" "tauv")
...
#  ifort -check all -traceback ${PROG_DIR}/RCM_sfc_${var[$v]}.f90 -o RCM_sfc_${var[$v]}.exe -I/usr/local/apps/netcdf4/${ncdf_v}/INTEL/${intel_v}/include -L/usr/local/apps/netcdf4/${ncdf_v}/INTEL/${intel_v}/lib -lnetcdff -lnetcdf
#  ifort -O3 ${PROG_DIR}/RCM_sfc_${var[$v]}.f90 -o RCM_sfc_${var[$v]}.exe -I/usr/local/apps/netcdf4/${ncdf_v}/INTEL/${intel_v}/include -L/usr/local/apps/netcdf4/${ncdf_v}/INTEL/${intel_v}/lib -lnetcdff -lnetcdf
   ifort -O2  ${PROG_DIR}/RCM_sfc_${var[$v]}.f90 -o RCM_sfc_${var[$v]}.exe -I/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/include -L/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/lib64 -lnetcdff -lnetcdf -L/usr/local/apps/hdf5-parallel/1.14.3/INTEL/2023.2/HPCX/2.17/lib64 -lhdf5hl_fortran -lhdf5_hl -lhdf5_fortran -lhdf5 -lm -lz
...
sbatch run_loop_tau.sh ${datebeg} ${dateend} ${year_lim}
