#!/bin/sh
#SBATCH --job-name=wrf-zlevta
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=8:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-zlevta.%j.out
#SBATCH --error=wrf-zlevta.%j.out
...
#                           TO CHANGE                           |
#----------------------------------------------------------------
RUN_DIR=$SCRATCH/ssp370/zlev_ta
mkdir -p ssp370/zlev_ta
...
#  ifort -O3 ${PROG_DIR}/RCM_zlev_ta.f90 -o RCM_zlev_ta.exe -I/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/include -L/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/lib -lnetcdff -lnetcdf
   ifort -O2  ${PROG_DIR}/RCM_zlev_ta_v1.f90 -o RCM_zlev_ta.exe -I/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/include -L/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/lib64 -lnetcdff -lnetcdf -L/usr/local/apps/hdf5-parallel/1.14.3/INTEL/2023.2/HPCX/2.17/lib64 -lhdf5hl_fortran -lhdf5_hl -lhdf5_fortran -lhdf5 -lm -lz
...
sbatch run_loop_zlev_ta.sh ${datebeg} ${dateend} ${year_lim}
