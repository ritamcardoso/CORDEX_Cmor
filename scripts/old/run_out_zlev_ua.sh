#!/bin/sh
#SBATCH --job-name=wrf-zlevua
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=20:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-zlevua.%j.out
#SBATCH --error=wrf-zlevua.%j.out
...
##### Submit as run_out_zlev_ua.sh date1 date2 date3  (yyyymmdd yyyymmdd yyyy)
...
#
    ifort -O3 ${PROG_DIR}/RCM_zlev_ua_v3.f90 -o RCM_zlev_ua.exe -I/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/include -L/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/lib -lnetcdff -lnetcdf
    ./RCM_zlev_ua.exe
#
...
sbatch run_loop_zlev_ua.sh ${datebeg} ${dateend} ${year_lim}
