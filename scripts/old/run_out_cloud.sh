#!/bin/sh
#SBATCH --job-name=wrf-cld
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=14:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-cld.%j.out
#SBATCH --error=wrf-cld.%j.out
...
#                           TO CHANGE                           |
#----------------------------------------------------------------
RUN_DIR=$SCRATCH/ssp370/cld
mkdir -p ssp370/cld

declare -a run=("d01")
declare -a var=("clt" "clh" "clm" "cll")
...
sbatch run_loop_cloud.sh ${datebeg} ${dateend} ${year_lim}
#sbatch run_out_wxtrm.sh ${datebeg} ${dateend} ${year_lim}
