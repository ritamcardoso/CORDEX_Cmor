#!/bin/sh
#SBATCH --job-name=wrf-xtrm
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=6:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-xtrm.%j.out
#SBATCH --error=wrf-xtrm.%j.out
...
declare -a run=("d01")
declare -a var=("tasmax" "tasmin" "sfcWindmax" "prcmax" "prncmax")
...
for(( j = ${yeari}; j <= ${yearf}; j++ )) ; do
 ...
 for (( r=0; r<${#run[@]}; r++)); do
  ...
  ln -sf ${DATA_DIR}/wrfxtrm_${run[$r]}_${j}* .
  ln -sf ${DATA_DIR}/wrfxtrm_${run[$r]}_${m}-01-01_00* .
  
  #...............................................................
  #                             var                              .
  #...............................................................
  for (( v=0; v<${#var[@]}; v++)); do
   cat ${HEADER_DIR}/header_xtrm_${run[$r]}.ini | sed s/_START_YY_/$START_YY/ | \
...
#  ifort -O3 ${PROG_DIR}/RCM_sfc_xtrm.f90 -o RCM_sfc_xtrm.exe -I/usr/local/apps/netcdf4/${ncdf_v}/INTEL/${intel_v}/include -L/usr/local/apps/netcdf4/${ncdf_v}/INTEL/${intel_v}/lib -lnetcdff -lnetcdf
   ifort -O2  ${PROG_DIR}/RCM_sfc_xtrm.f90 -o RCM_sfc_xtrm.exe -I/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/include -L/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/lib64 -lnetcdff -lnetcdf -L/usr/local/apps/hdf5-parallel/1.14.3/INTEL/2023.2/HPCX/2.17/lib64 -lhdf5hl_fortran -lhdf5_hl -lhdf5_fortran -lhdf5 -lm -lz
...
sbatch run_loop_wxtrm.sh ${datebeg} ${dateend} ${year_lim}
