 ...
 for (( r=0; r<${#run[@]}; r++)); do
  ...
  for (( v=0; v<${#var[@]}; v++)); do
   cat ${HEADER_DIR}/header_${run[$r]}.ini | sed s/_START_YY_/$START_YY/ | \
                                             sed s/_END_YY_/$END_YY/  > ${RUN_DIR}/header_soil_${run[$r]}
   cat header_soil_${run[$r]} > outputlist.inp
   echo "#" >> outputlist.inp
...
#  ifort -traceback -check all ${PROG_DIR}/RCM_sfc_${var[$v]}.f90 -o RCM_sfc_${var[$v]}.exe -I/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/include -L/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/lib -lnetcdff -lnetcdf
