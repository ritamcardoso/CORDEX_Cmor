#!/bin/sh
#SBATCH --job-name=wrf-1zlevua
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=15:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-1zlevua.%j.out
#SBATCH --error=wrf-1zlevua.%j.out
...
#                           TO CHANGE                           |
#----------------------------------------------------------------
RUN_DIR=$SCRATCH/ssp370/zlev_ua1
mkdir -p ssp370/zlev_ua1

declare -a run=("d01")
declare -a var=("ua200m" "ua250m" "ua300m")
...
#  ifort -O3 ${PROG_DIR}/RCM_zlev_ua.f90 -o RCM_zlev_ua.exe -I/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/include -L/usr/local/apps/netcdf4/${netcdf_v}/INTEL/${intel_v}/lib -lnetcdff -lnetcdf
   ifort -O2  ${PROG_DIR}/RCM_zlev_ua.f90 -o RCM_zlev_ua.exe -I/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/include -L/usr/local/apps/netcdf4-parallel/4.9.2/INTEL/2023.2/HPCX/2.17/lib64 -lnetcdff -lnetcdf -L/usr/local/apps/hdf5-parallel/1.14.3/INTEL/2023.2/HPCX/2.17/lib64 -lhdf5hl_fortran -lhdf5_hl -lhdf5_fortran -lhdf5 -lm -lz
...
sbatch run_loop_zlev_ua1.sh ${datebeg} ${dateend} ${year_lim}
