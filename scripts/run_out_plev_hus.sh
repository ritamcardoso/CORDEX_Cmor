#!/bin/sh
#SBATCH --job-name=wrf-hus
#SBATCH --qos=nf
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --time=14:00:00
#SBATCH --hint=nomultithread
#SBATCH --output=wrf-hus.%j.out
#SBATCH --error=wrf-hus.%j.out
...
# 2. Compile program
#
   $FC $FFLAGS ${PROG_DIR}/RCM_plev_hus.f90 ${MOD_NAME}.o ${SUB_NAME}.o -o RCM_plev_hus.exe $ALL_LIBS
...
# sbatch --file=slurm_common.opts run_loop_plev_hus.sh ${datebeg} ${dateend} ${year_lim}
 sbatch --file=slurm_common.opts run_zlev_hus.sh ${datebeg} ${dateend} ${year_lim}
fi
