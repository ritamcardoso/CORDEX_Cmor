...
allocate(p(nlon, nlat, nz), pb(nlon, nlat, nz), press(nlon, nlat, nz+1))
allocate(qc(nlon, nlat, nz), qi(nlon, nlat, nz), qr(nlon, nlat, nz), qs(nlon, nlat, nz))
...
!
        ...
        press(:,:,nz+1)=p_top
        do k=2,nz
          press(:,:,k) = 0.5((p(:,:,k-1) + pb(:,:,k-1))+(p(:,:,k) + pb(:,:,k)))
        enddo
!
...
!
do k=1,nz
  outvar(:,:)=outvar(:,:)+(wrfv3D(:,:,k)/(wrfv3D(:,:,k)+1))*(press(:,:,k)-press(:,:,k+1)
enddo
