...
!-----------------------------------------------------------------------
!
subroutine read_global_proc
    use datvar_s
    implicit none

    integer :: inperr, ichlef
    character(1000) :: line
    character(30)  :: keyword

    ! 1. Open the global attributes list
    open(10, file='global_data.inp', status='old', iostat=inperr)
    if (inperr /= 0) then
        write(*,*) 'Warning: globallist.inp not found. Global attributes will be empty.'
        return
    endif
    ...
    ! Use unit 27 (scratch) to handle value conversion
    ...
    do
        ...
        keyword = adjustl(line(1:ichlef-1))
        call toupcase(keyword)

        ! Put the value part (after '=') into scratch
        rewind(27)
        write(27, '(a)') adjustl(line(ichlef+1:1000))
        rewind(27)

        ! Assign to the lowercase variables in datvar_s
        ...
        case ('CONTACT')
            read(27,*) contact
        case ('CORDEX_DOMAIN')
            read(27,*) cordex_domain
        case ('DOMAIN_NAME')
            read(27,*) domain_name
        case ('DOMAIN_ID')
            read(27,*) domain_id
        case ('DRIVING_INST_ID')
            read(27,*) driving_inst_id
        case ('DRIVING_SOURCE_ID')
            read(27,*) driving_source_id
        case ('DRIVING_SOURCE')
            read(27,*) driving_source
        case ('DRIVING_VARIANT')
            read(27,*) driving_variant
        case ('DEXP_ID')
            read(27,*,iostat=inperr) dexp_id    
        case ('DEXP')
            read(27,*,iostat=inperr) adexp      
        case ('FURTHER_INFO_URL')
            read(27,*) further_info_url
        case ('GRID_DESC')
            read(27,*) grid_desc
        case ('INSTITUTION')
            read(27,*) institution
        case ('INSTITUTION_ID')
            read(27,*) institution_id
        case ('LABEL')
            read(27,*) label
        case ('LABEL_EXT')
            read(27,*) label_ext
        ...
        case ('SOURCE_TYPE')
            read(27,*) source_type
        case ('VERSION_REAL')
            read(27,*) version_real
        case ('SIM_START_DATE')
            read(27,*) sim_start_date
        case ('DX_VAL')
            read(27,*) dx_val
        case ('DY_VAL')
            read(27,*) dy_val
    ...
    write(*,*) '>>> Global metadata loaded from globallist.inp'

end subroutine read_global_proc
!
!-----------------------------------------------------------------------
!
subroutine readdata
    ! Reads the input data and the data option switches from outputlist.inp
    use datvar_s
    implicit none
    ...
    ok_geog=.false.    ; ok_wrffile=.false. 
...
contains
    ...
    subroutine abort_msg(var)
        ...
        write(*,*) 'Aborted: Missing required input in outputlist.inp: ', var
        stop
