...
!
integer, parameter :: nseason=4,tmonths=12,iniyear=1980
integer :: nlon,nlat,nz,xoffset,yoffset,level,nsoil
integer :: ish,isd,is3,is6,issh,iss3,iss6,issd,isdm
integer :: yeari,yearf,nmonths,imonth,ndays,iday,pyear,loop_year
...
integer, dimension(nseason) :: sedays,sedays1,sedays2
!
integer :: ncid,varid,status
integer :: latid,lonid,timeid,levid,timedim,latdim,londim,levdim
!
real :: cp,rcp,tc,rhours,factor
...
real, dimension(:,:), allocatable :: lon,lat,tlon,tlat
real, dimension(:,:), allocatable :: outvar,outvar_i
real, dimension(:,:,:), allocatable :: outvar_h,outvar_3,outvar_6,outvar_d,outvar_m,outvar_d_max
real, dimension(:,:), allocatable  :: wrfv
real, parameter :: earthrad=6372.795,epsilon=0.6220,Rd=287.04,g=9.81,gamma=0.0065,pconst=1000.
!
!
character*2 ahouri,adayi,amonthi,ahourf,adayf,amonthf,ahour,aday,amonth,aiday,aimonth,dom,mm,dd,hh,mn,ss
character*4 ayear,ayearf,ayeari,aiyear,yyyy
character*10 vaid,varname,vunts,vunits
character*25 wrfile,wrffile,geo,geog,wrfvar,wrfvarname,subrname,subname
character*100 longname,standardname,cellmethods,timeunits,frequency,experiment,dexperiment,creationdate
character*100 lname,stname,cmethods,tunts,freq,exper,dexper,cdate
character*400 infile,filename,outfile,outdom,fnameout,dir,dir2,gfile,geofile,aexp,adexp
character, dimension(nseason) :: snames*3
character, dimension(tmonths) :: aname*3
character(8) :: date
character(10) :: times
character(5) :: zone
!
data height/2./
data snames/'Win','Spr','Sum','Aut'/
data aname/'Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'/
data days1/31,28,31,30,31,30,31,31,30,31,30,31/
data days2/31,29,31,30,31,30,31,31,30,31,30,31/
data sedays1/90,92,92,91/                                                                     ! Number of days in each season
data sedays2/91,92,92,91/                                                                     ! Number of days in each season in a
!
end module datvar
!
program read_wrfout
...
call ncerror(status,'getting var id')
!
status=nf90_get_var(ncid,varid,wrfv,(/xoffset,yoffset/),(/nlon,nlat/),(/1,1/))
...
status = nf90_put_att(ncid, uVarID, "grid_mapping","crs")
