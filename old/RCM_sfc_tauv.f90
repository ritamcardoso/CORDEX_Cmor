...
real, dimension(:,:), allocatable  :: wrfv2D,outvar,outvar_i,u_s,v_s,wind10,ust,t2,q2,ua,va,ro,psf
...
allocate(wind10(nlon,nlat))
allocate(ua(nlon,nlat))
allocate(va(nlon,nlat))
allocate(ro(nlon,nlat))
...
do ix=1,nlon
  do iy=1,nlat
!
    ua(ix,iy)=(u_s(ix,iy)*cosalp(ix,iy))-(v_s(ix,iy)*sinalp(ix,iy))
    va(ix,iy)=(v_s(ix,iy)*cosalp(ix,iy))+(u_s(ix,iy)*sinalp(ix,iy))
    wind10(ix,iy)=sqrt(u_s(ix,iy)**2+v_s(ix,iy)**2)
!
    ro(ix,iy)=(psf(ix,iy)/(Rd*t2(ix,iy)))*(1-((q2(ix,iy)*(1-epsilon))/(epsilon+q2(ix,iy))))
!
    outvar(ix,iy)=ro(ix,iy)*(ust(ix,iy)**2./wind10(ix,iy))*va(ix,iy)
!
  enddo
enddo
!
