...
real, dimension(:,:), allocatable  :: wrfv2D,outvar,outvar_i,psf,t_l,pres,phi_s
real, dimension(:,:,:), allocatable :: outvar_h,wrfv3D
real, dimension(:,:,:), allocatable  :: p,pb,press,phb,ph,phi,t
...
!allocate(press(nlon,nlat,1))
!allocate(phi(nlon,nlat,1))
allocate(t(nlon,nlat,1))
allocate(pres(nlon,nlat))
...
do j = 1,nlon
  do k = 1,nlat
    ...
    if (abs(phi_s(j,k)).lt.0.001) then
      ...
      t_surf = t_l(j,k) + gamma * (Rd/g) * ( psf(j,k)/pres(j,k) - 1.0 ) * t_l(j,k)
