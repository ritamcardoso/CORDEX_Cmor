...
!
real :: rcp,tc,rhours,factor,heightl,presl
...
real, dimension(:,:), allocatable :: lon,lat,tlon,tlat,cosalp,sinalp,landmask,sftlf,hgt
real, dimension(:,:), allocatable  :: wrfv2D,outvar,outvar_i
real, dimension(:,:,:), allocatable :: outvar_h,wrfv3D
real, dimension(:,:,:), allocatable  :: p,pb,press,phb,ph,phi,ua,va,zhgt,u,v
real, parameter :: earthrad=6372.795,epsilon=0.6220,cp=1004.0,Rd=287.04,g=9.81,gamma=0.0065,pconst=1.0e5
!
...
!
! Compute temperature at pressure levels
!
...
!
        ...
        status=nf90_get_var(ncid,varid,wrfv3D,(/xoffset,yoffset,1/),(/nlon,nlat,nz/),(/1,1,1/))
        call ncerror(status,'reading '//wrfvar)
!
! Compute variable
!
        wrfv3D=(wrfv3D+290.)*(press/pconst)**rcp
!
        deallocate(press)
!
        allocate(ph(nlon,nlat,nzt))
        allocate(phb(nlon,nlat,nzt))
        allocate(phi(nlon,nlat,nzt))
!
        status=nf90_inq_varid(ncid,'PH',varid)
        call ncerror(status,'getting var id')

        status=nf90_get_var(ncid,varid,ph,(/xoffset,yoffset,1/),(/nlon,nlat,nzt/),(/1,1,1/))
        call ncerror(status,'reading '//'PH')

        status=nf90_inq_varid(ncid,'PHB',varid)
        call ncerror(status,'getting var id')

        status=nf90_get_var(ncid,varid,phb,(/xoffset,yoffset,1/),(/nlon,nlat,nzt/),(/1,1,1/))
...
!
        slope=(wrfv3D(isx,isy,iz+1)-wrfv3D(isx,isy,iz))/(zhgt(isx,isy,iz+1)-zhgt(isx,isy,iz))
