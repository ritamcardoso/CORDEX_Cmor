...
real, dimension(:,:), allocatable  :: wrfv2D,outvar,outvar_i,runsf
...
!
        ...
        call ncerror(status,'opening file')
!
        status=nf90_inq_varid(ncid,'RUNSF',varid)
        call ncerror(status,'getting var id')

        status=nf90_get_var(ncid,varid,runsf,(/xoffset,yoffset/),(/nlon,nlat/),(/1,1/))
        call ncerror(status,'reading '//'RUNSF')    
!
...
!
        ...
        outvar_h(:,:,issh)=((wrfv2D(:,:)+runsf(:,:))+outvar_i(:,:))/2.
!
        outvar_i=wrfv2D+runsf
