...
real, dimension(:,:), allocatable  :: wrfv2D,i_wrfv2D,outvar,outvar_i,psf
real, dimension(:,:,:), allocatable :: outvar_h,wrfv3D,qc,qi,qr,qs
...
allocate(qc(nlon,nlat,nz))
allocate(qi(nlon,nlat,nz))
allocate(qr(nlon,nlat,nz))
allocate(qs(nlon,nlat,nz))
allocate(p(nlon,nlat,nz))
allocate(pb(nlon,nlat,nz))
allocate(press(nlon,nlat,nz))
...
!
          ...
          status=nf90_get_var(ncid,varid,p,(/xoffset,yoffset,1/),(/nlon,nlat,nz/),(/1,1,1/))
          call ncerror(status,'reading '//'P')

          status=nf90_inq_varid(ncid,'PB',varid)
          call ncerror(status,'getting var id')

          status=nf90_get_var(ncid,varid,pb,(/xoffset,yoffset,1/),(/nlon,nlat,nz/),(/1,1,1/))
...
!
        ...
        call ncerror(status,'getting var id')
!
        status=nf90_get_var(ncid,varid,qc,(/xoffset,yoffset,1/),(/nlon,nlat,nz/),(/1,1/))
        call ncerror(status,'reading '//wrfvar)
!
        status=nf90_inq_varid(ncid,'QICE',varid)
        call ncerror(status,'getting var id')
!
        status=nf90_get_var(ncid,varid,qi,(/xoffset,yoffset,1/),(/nlon,nlat,nz/),(/1,1/))
        call ncerror(status,'reading '//'QICE')
!
        status=nf90_inq_varid(ncid,'QRAIN',varid)
        call ncerror(status,'getting var id')
!
        status=nf90_get_var(ncid,varid,qr,(/xoffset,yoffset,1/),(/nlon,nlat,nz/),(/1,1/))
        call ncerror(status,'reading '//'QRAIN')
!
        status=nf90_inq_varid(ncid,'QSNOW',varid)
        call ncerror(status,'getting var id')
!
        status=nf90_get_var(ncid,varid,qs,(/xoffset,yoffset,1/),(/nlon,nlat,nz/),(/1,1/))
        call ncerror(status,'reading '//'QSNOW')
!
        status=nf90_close(ncid)
        call ncerror(status,'closing file')        
!
...
!  1st model level
!
     pwd_s=(wrfv3D(ix,iy,1)/(wrfv3D(ix,iy,1)+1))*(psf(ix,iy)-0.5*(press(ix,iy,1)+press(ix,iy,2)))
!
!  From the 2nd to the top-1 level
!
     do k=2,nz-1
       pwd_s=pwd_s+(wrfv3D(ix,iy,k)/(wrfv3D(ix,iy,k)+1))*(0.5*(press(ix,iy,k-1)-press(ix,iy,k+1)))
     enddo
!
!  Last model level
!
     pwd_s=pwd_s+(wrfv3D(ix,iy,nz)/(wrfv3D(ix,iy,nz)+1))*(0.5*(press(ix,iy,nz-1)+press(ix,iy,nz))-2000.)
