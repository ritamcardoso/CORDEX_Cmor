...
integer :: ish,issh
integer :: yeari,yearf,iniyear,nmonths,imonth,ndays,iday,loop_year
integer :: year,month,day,mydays,hour,dhour,nhours,yhours,ihour
integer :: ntime
...
!
        do iz=1,nz
          zhgt(:,:,iz)=(((phi(:,:,iz)+phi(:,:,iz+1))/2.)/g)-hgt(:,:)
        enddo 
!
        deallocate(ph)
        deallocate(phb)
        deallocate(phi)
!
        allocate(ua(nlon_u,nlat,nz))
        allocate(va(nlon,nlat_v,nz))
!
