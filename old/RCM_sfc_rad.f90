...
allocate(i_wrfv2D(nlon,nlat))

fname='I_'//trim(wrfvar)
iwrfvar=trim(fname)
write(*,*)iwrfvar,wrfvar
!
it=0
yearf=yeari+1
!
...
!
        ...
        status=nf90_inq_varid(ncid,iwrfvar,varid)
        call ncerror(status,'getting var id')

        status=nf90_get_var(ncid,varid,i_wrfv2D,(/xoffset,yoffset/),(/nlon,nlat/),(/1,1/))
        call ncerror(status,'reading '//iwrfvar)
!
        status=nf90_close(ncid)
        call ncerror(status,'closing file')
!
...
!
        ...
        outvar_a(:,:)=nint(100.d0*((outvar(:,:)-outvar_i(:,:))/3600.))
...
do ix=1,nlon
  do iy=1,nlat
    outvar(ix,iy)=wrfv2D(ix,iy)+i_wrfv2D(ix,iy)*1.e9
  enddo
enddo
!
end subroutine calc_rad
!
!----------------------------------------------------------------------------------------------------------------------
!
