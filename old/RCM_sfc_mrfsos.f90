...
integer :: ish,issh
integer :: yeari,yearf,iniyear,nmonths,imonth,ndays,ihour,iday,loop_year
...
real, dimension(:,:,:), allocatable  :: p,pb,press,phb,ph,phi,smois
...
allocate(smois(nlon,nlat,nsoil))
...
!----------------------------------------------------------------------------------------------------------------------
!
subroutine calc_mrfsos
use datvar
real, dimension(nlon,nlat) :: temp
!
do ix=1,nlon
  do iy=1,nlat
!
    temp(ix,iy)=((0.1*wrfv3D(ix,iy,1)))*1000.
!
    outvar(ix,iy)=temp(ix,iy)*landmask(ix,iy)
!
  enddo
enddo
!
end subroutine calc_mrfsos 
!
!----------------------------------------------------------------------------------------------------------------------
!
