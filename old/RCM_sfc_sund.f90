...
real, dimension(:,:), allocatable  :: wrfv2D,outvar,outvar_i,sund
...
!----------------------------------------------------------------------------------------------------------------------
!
subroutine calc_sund
use datvar
!
do iy=1,nlat
  do ix=1,nlon
    if(wrfv2D(ix,iy) >= 120. .and. outvar_i(ix,iy) >= 120.)then
      sund(ix,iy)=3600.
    elseif(wrfv2D(ix,iy) < 120. .and. outvar_i(ix,iy) < 120.)then
      sund(ix,iy)=0.
    else
      slope=(wrfv2D(ix,iy)-outvar_i(ix,iy))/3600.
      if( slope > 0 ) then
        sund(ix,iy)=3600.-(120. - outvar_i(ix,iy)) / slope
      elseif( slope < 0 ) then
        sund(ix,iy)=(120. - outvar_i(ix,iy)) / slope
      endif
    endif
  enddo
enddo
!
end subroutine calc_sund
!
!----------------------------------------------------------------------------------------------------------------------
!
